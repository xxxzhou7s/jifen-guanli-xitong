  ...
  if(url.origin===self.location.origin && (url.pathname.endsWith('/app.html')||url.pathname==='/'||url.pathname.endsWith('/index.html'))){
    event.respondWith((async()=>{
      try{
        ...
        const css='<link rel="stylesheet" href="/mobile.css?v=7"><link rel="stylesheet" href="/mobile-fix.css?v=1">';
        const patched=text.includes('mobile-fix.css')?text:text.replace('</head>',css+'</head>');
